...
function renderDashboard(data) {
  ...
  $("provider-list").innerHTML = data.providers.length ? data.providers.map((item) => `<article class="provider"><span class="health health-${escapeHtml(item.status)}" aria-hidden="true"></span><span><strong>${escapeHtml(item.id)}</strong><small>${escapeHtml(item.type)} &middot; ${escapeHtml(item.standby ? "standby" : item.status)}${item.version ? ` &middot; ${escapeHtml(item.version)}` : ""}</small><small>${escapeHtml(item.path || item.error || "No executable detail")}</small><small>Execution ${item.execution_enabled ? "enabled" : "disabled"} &middot; Roles: ${escapeHtml(item.allowed_roles.join(", ") || "simulation only")}</small><details><summary>Redacted health detail</summary><pre>${escapeHtml(JSON.stringify(item.health_details, null, 2))}</pre></details></span></article>`).join("") : empty("No providers configured");
...
async function selectWorkItem(taskId) {
  ...
  $("work-detail").innerHTML = `
    ...
    <div class="command-bar"><label>Claim as<input id="claim-agent" value="${escapeHtml(item.assignee || "coding-worker-gemini")}" aria-label="Agent ID for claim"></label><button type="button" data-command="claim">Claim</button>${item.kind === "task" ? `<button type="button" data-command="run">Run simulation</button>` : `<span class="panel-copy">Only leaf tasks can run; this ${escapeHtml(item.kind)} is a planning item.</span>`}<button type="button" class="danger" data-command="archive">Archive item</button></div>`;
  await loadWork();
}
...
async function showRun(runId) {
  ...
  try {
    ...
    const stages = detail.artifacts.map((artifact, index) => {
      ...
      const evidence = payload.acceptance_evidence || payload.criteria_evidence || payload.evidence || payload.output || "No structured evidence";
      const errors = payload.errors || payload.error || "None recorded";
      return `<article class="stage-card"><div class="stage-number">${index + 1}</div><div><div class="stage-head"><h3>${escapeHtml(artifact.stage)}</h3>${badge(artifact.status)}</div><p>${escapeHtml(artifact.agent_id)} &middot; ${escapeHtml(artifact.provider)}</p><dl class="evidence"><div><dt>Verdict</dt><dd>${escapeHtml(payload.verdict || review?.verdict || "Not reviewed")}</dd></div><div><dt>Evidence</dt><dd>${escapeHtml(typeof evidence === "string" ? evidence : JSON.stringify(evidence))}</dd></div><div><dt>Errors</dt><dd>${escapeHtml(typeof errors === "string" ? errors : JSON.stringify(errors))}</dd></div></dl></div></article>`;
    }).join("");
