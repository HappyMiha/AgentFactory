...
The [Tool Gateway](tool-gateway.md) resolves immutable descriptors declaring input/output schemas, side effects, risk, capabilities, timeout, and evidence outputs. Invocation authority is the intersection of mission, role, and policy allowlists; dynamic MCP discovery is stored as evidence and cannot widen that intersection. Connector versions and install, health, failure, disable, upgrade, and removal transitions are audited. Mutation-capable production connector versions require human registration approval.

The [credential broker](credential-broker.md) retains secret values only in its in-memory lease vault. Persistent records contain an opaque handle and tenant/mission/tool/operation/expiry scope but no secret or secret digest. The injection firewall rejects secret/handle material in prompts or tool arguments; exact-scoped execution receives one allowlisted environment entry, and output or exceptions are recursively redacted before evidence/audit persistence. Revocation removes the value and blocks later use while retaining non-secret lifecycle evidence.

The [prompt-injection defense](prompt-injection-defense.md) maintains immutable indirect-injection, authority-escalation, secret-extraction, tool-abuse, artifact-poisoning, and cross-tenant cases. Deterministic matches create linked attempts, tripwires, quarantined content, and material incidents. Quarantined content has one admission gate for accepted context, memory, artifacts, and downstream execution; only a named human security reviewer may release it or close the incident. Evidence-tampering reports bind the actor, attempt, original digest, evidence identity, and exact affected criterion while the accepted-evidence database lock preserves the original.

The [coordination pattern service](coordination-patterns.md) stores immutable manifests for parallel, generator-critic, quorum, debate, tournament, and red/blue workflows. Each manifest fixes participants and model-aware reviewer pools, independence rules, turn/token/cost ceilings, arbitration, termination, and required evidence. Every contribution is typed and content-addressed before it can influence deterministic arbitration. Reviewer selection excludes the producer agent and model and rotates least-used eligible reviewers. Completed outcomes retain every explicit dissent record; limit breaches terminate fail closed without admitting the over-budget contribution.

The [ADR governance service](adr-governance.md) requires context, alternatives, decision, consequences, affected workflow contracts, evidence, material domains, and an explicit approval state. Its immutable pre-approval analysis enumerates tasks, context packages, policies, evaluations, artifacts, deployment assumptions, and changed Blueprint sections. Only the named human architecture owner can approve that exact decision/impact pair. Application then creates the new Blueprint version, versioned workflow contracts, and target-by-target propagation ledger in one SQLite transaction; any failure rolls everything back and preserves the prior approved/authorized Blueprint.

The [pack SDK and manager](pack-lifecycle.md) canonicalizes manifests for domain, capability, connector, policy, evaluation, and UI extensions. HMAC-SHA-256 signing material remains in memory while a human-approved trust-root fingerprint is durable. Core compatibility, declared/requested permissions, active dependencies, complete passing evaluation results, and privileged administrator approval are checked before a version becomes active. Pack payloads remain data rather than edits to factory core; immutable versions, qualifications, and lifecycle events preserve mission history while the active pointer can be disabled or rolled back.

The [Software Engineering reference pack](reference-pack.md) composes those pack controls with the already-qualified AF-048/049/050/051/052/020/053 contracts. Its manifest contains only contract references and role-pack identity, while a separate immutable release envelope links requirements, tasks, Blueprint decisions, ADRs, test evidence, and accepted independent-review references. Dependency, security, and rollback evidence are reproducible records; publishing or protected-branch integration remains under the configured human release authority.

The [managed worktree service](managed-worktrees.md) is the only component allowed to create task or Autonomous Mission epoch Git worktrees. Task paths bind an approved base SHA to a live fenced assignment; [epoch paths](development/mission-epoch-worktrees.md) bind an immutable mission/epoch/checkpoint authority before Git mutation. Both detect dirty, missing, orphaned, or conflicting state without rewriting history; task cleanup still requires terminal state plus the persisted retention deadline, while prior epoch branches/worktrees remain preserved.
...
The [execution telemetry service](execution-telemetry.md) assigns one correlation root to task/workflow, Hermes and Codex sessions, worktree, validator, approval, candidate, evaluation, and delivery identities. Immutable samples retain duration, retries, tokens, estimated cost, tool calls, and terminal reason. AF-027 exports that same correlation root through an OpenTelemetry-compatible payload, derives queue/wait/duration/failure/iteration/orphan metrics, and records provider-reported or estimated cost in an idempotent ledger. Durable threshold actions can notify, reroute, pause, or require approval; increasing a hard mission budget requires an immutable human budget authorization.
