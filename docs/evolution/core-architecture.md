...
Три набори: видимі development tasks; sealed adaptive selection set; окремий final confirmation set. Optimizer бачить дозволений діагностичний feedback, а не приховані відповіді. Залучення holdout до skill memory забруднює набір: це lineage event і причина створити новий test set. Один і той самий тест не стає новим лише через зміну ID.

**Q08 уточнив пілотну гіпотезу:** [дві non-game сім’ї](non-game-evaluation.md) — зовнішня bounded software maintenance і requirements-to-decision planning із різними work products та root/source corpora. Контекст, tool use, repair, recovery і user-facing evidence є cross-cutting slices. Початкове припущення 60 paired tasks означає 60 root-task pairs одного future confirmation block, орієнтовно 30 на family; D/S preparation та search — окремий обсяг. Три stochastic повтори — гіпотеза noise design, не три незалежні tasks. Усі числа потребують precision/resource review до запуску; actual corpus/qualification/compute цього проходу немає. Root ancestry, exposure, reference R0/Rn, повтори/looks і negative results лишаються явними.
