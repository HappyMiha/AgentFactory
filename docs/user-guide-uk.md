...
### Що відбувається зі звичайним описом гри

Завантажений текст без Markdown не втрачає змісту. Оригінальні байти й витягнутий
текст зберігаються поруч із пропозицією, а розбір лишається детермінованим — жодна
модель не викликається.

- Кома розділяє вимоги, які ви перелічили: «Кіт збирає монети, три життя» дає дві
  окремі редаговані вимоги. Сполучники «та», «і», `and` не розділяють нічого, бо
  вони так само з'єднують частини однієї фрази.
- Підрядне речення («…кота, який збирає монети») лишається в тій самій вимозі.
- Кожен пункт списку — окрема вимога; маркер списку прибирається.
- Розділ без жодної вимоги не стає готовим планом: попередній перегляд ставить
  конкретне запитання, а `plan_ready` лишається `false`, доки ви не відповісте.
- Назва, взята з імені файлу, теж потребує підтвердження.

Ви редагуєте попередній перегляд перед імпортом. Детермінований імпорт, пропозиція
AI та підтверджений план — це три різні стани, і інтерфейс не називає один одним.
