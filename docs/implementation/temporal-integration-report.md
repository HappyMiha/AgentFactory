...
Syntax, patch hygiene, Compose rendering, targeted API tests, historical replay, forced three-run continue-as-new with Worker replacement, operation-journal kill-point and recovery tests, real-Git epoch branch/worktree collision and restart tests, official Temporal SDK test-server tests, and the entire existing suite were exercised. Final complete-suite result after AF-AMM-019: **409 tests run: 408 passed and 1 opt-in Docker test skipped by default**.
