...
AF-AMM-007 through AF-AMM-016 establish the source, role-assignment, proposal-generation, verification, exact human approval boundary, durable pre-approval orchestration, sequential post-approval child delivery, its mission-wide control fence, and owner-authorized checkpoint/revision epoch handoff. Pre-approval operations do not authorize repository, environment, service, or Git mutation, and they do not create executable `work_items`; those records appear only after the exact approval authority has been revalidated.
