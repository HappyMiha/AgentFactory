...
| Autonomous Mission Mode | Parent orchestration foundation ready | Durable domain/authorization/planning/approval records plus the atomic exact-backlog start and identifier-only `AutonomousMissionWorkflow` contract are implemented; domain Activities, child scheduling, and execution slices remain in progress. |
